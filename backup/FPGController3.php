...
class FPGController extends Controller
{
    ...
    public function proses1(Request $request)
    {
        if (Auth::user()->id_priv == 1) {
            ...
            // Setelah mengumpulkan data dalam $filteredGrupdByTgl
    ...
    public function hasil(Request $request)
    {
        ...
        $itemNames = array_flip($itemInitials);
        ...
        foreach ($conditionalFPTreeAndPatterns as $item => $data) {
            foreach ($data['frequentPatterns'] as $pattern) {
                ...
                if (count($matches) === 3) {
                    ...
                    $antecedent = implode(', ', array_map(function($item) use ($itemNames) {
                        return $itemNames[$item] . ' (' . $item . ')';
                    }, $patternItems));
                    $formattedRule = "Jika " . $antecedent . " maka " . $itemNames[$consequent] . ' (' . $consequent . ')';

                    $formattedFrequentPatterns[] = [
                        'item' => $formattedRule,
                        'pattern' => implode(', ', array_map(function($item) use ($itemNames) {
                            return $itemNames[$item] . ' (' . $item . ')';
                        }, explode(', ', $matches[1]))),
                        'count' => $count,
                    ...
                    if (count($patternItems) > 1) {
                        foreach ($patternItems as $antecedentItem) {
                            ...
                            $antecedentPatternString = implode(', ', array_map(function($item) use ($itemNames) {
                                return $itemNames[$item] . ' (' . $item . ')';
                            }, $antecedentPattern));
                            ...
                            if ($antecedentCount > 0) {
                                ...
                                $confidence = 0; // atau nilai default lain yang sesuai
                            }

                            $confidenceResults[] = [
                                'rule' => "Jika " . $antecedentPatternString . " maka " . $itemNames[$antecedentItem] . ' (' . $antecedentItem . ')',
                                'support' => number_format($supportPercentage, 2),
                            ...
                            if ($confidence >= $minConfidence) {
                                $conclusionResults[] = [
                                    'rule' => "Jika " . $antecedentPatternString . " maka " . $itemNames[$antecedentItem] . ' (' . $antecedentItem . ')',
                                    'confidence' => number_format($confidence, 2),
                                    'description' => "Jika pelanggan membeli " . $antecedentPatternString . ", maka kemungkinan besar akan membeli " . $itemNames[$antecedentItem] . ' (' . $antecedentItem . ').'
                                ];
                            }
                        }
                    }
